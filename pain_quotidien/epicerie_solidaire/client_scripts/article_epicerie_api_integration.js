/**
 * Client Script pour Article Epicerie - VERSION CLIENT-SIDE
 * 
 * Appels APIs directs depuis le navigateur:
 * - Open Food Facts
 * - Open Products Facts
 * - Open Price
 */
        ...
        callback: function(r) {
            if (r.message && r.message.length > 0) {
                ...
                frappe.msgprint({
                    ...
                    indicator: 'blue',
                    message: __(`
                        <p><strong>Un article avec ce code-barre existe déjà !</strong></p>
                        <p>📦 <strong>${existing.nom_article}</strong></p>
                        <p>📊 Stock actuel: <strong>${existing.stock_actuel || 0}</strong></p>
                        <p><a href="/app/article-epicerie/${existing.name}">Ouvrir l'article</a></p>
                    `)
                });
...
/**
 * Refresh données API (article existant)
 */
function refresh_from_api(frm) {
    ...
    frappe.confirm(
        ...
        function() {
            fetch_from_apis_clientside(frm, frm.doc.code_barre);
        }
    );
}
...
/**
 * Appels APIs directs depuis navigateur
 */
async function fetch_from_apis_clientside(frm, barcode) {
    
    frappe.show_alert({
        message: __('🔍 Recherche dans Open Food Facts...'),
        indicator: 'blue'
    }, 3);
    ...
    let result = {
        ...
        image_url: null,
        categories_api: null,
        suggested_rayon: null,
        suggested_famille: null,
        prix_moyen_eur: null,
        prix_last_update: null
    };
    ...
    try {
        ...
        if (food_data.found) {
            ...
            // ÉTAPE 2: Open Products Facts
            frappe.show_alert({
                message: __('🔍 Recherche dans Open Products Facts...'),
                indicator: 'blue'
            }, 3);
        ...
        // ÉTAPE 3: Open Price (toujours)
        frappe.show_alert({
            message: __('💰 Recherche prix...'),
            indicator: 'blue'
        }, 2);
        ...
        frappe.msgprint({
            title: __('Erreur'),
            indicator: 'red',
            message: __('Erreur lors des appels APIs: ' + error.message)
        });
    }
}
...
async function fetch_openfoodfacts(barcode) {
    ...
    try {
        ...
        if (product.categories_tags) {
            result.categories_api = product.categories_tags.slice(0, 5).join(', ');
        }
...
async function fetch_openprice(barcode) {
    ...
    try {
        ...
        // Calculer prix moyen EUR uniquement
        const eur_prices = data.items
        ...
        if (eur_prices.length > 0) {
            ...
            if (data.items[0].date) {
                result.prix_last_update = data.items[0].date;
            }
        }
...
/**
 * Mapper catégories vers Rayon + Famille
 * Retourne: {rayon: 'Alimentaire', famille: 'Boissons'}
 */
function map_category(categories_str, type) {
    
    if (!categories_str) {
        return type === 'food' 
            ? {rayon: 'Alimentaire', famille: 'Autre Alimentaire'}
            : {rayon: 'Non Alimentaire', famille: 'Autre Non Alimentaire'};
    }
    ...
    if (type === 'food') {
        const mappings = {
            'Fruits et Légumes': ['fruit', 'vegetable', 'légume', 'legume'],
            'Produits Laitiers': ['dairy', 'cheese', 'yogurt', 'milk', 'lait', 'fromage'],
            'Viandes et Poissons': ['meat', 'fish', 'viande', 'poisson'],
            'Boulangerie': ['bread', 'bakery', 'pain', 'boulangerie'],
            'Épicerie Sèche': ['pasta', 'rice', 'pâtes', 'riz', 'noodles', 'flour', 'farine'],
            'Conserves': ['canned', 'preserved', 'conserve', 'tinned'],
            'Surgelés': ['frozen', 'surgelé', 'ice-cream', 'glace'],
            'Boissons': ['beverage', 'drink', 'water', 'juice', 'soda', 'boisson', 'coffee', 'tea'],
            'Petit Déjeuner': ['breakfast', 'cereals', 'jam', 'honey', 'confiture', 'miel'],
            'Condiments': ['sauce', 'condiment', 'ketchup', 'mayonnaise', 'mustard', 'vinegar'],
            'Snacks': ['snack', 'chips', 'crisp', 'biscuit', 'cookie'],
            'Plats Préparés': ['prepared', 'ready', 'meal', 'plat', 'pizza']
        };
            ...
            for (let tag of tags) {
                for (let keyword of keywords) {
                    if (tag.includes(keyword)) {
                        return {rayon: 'Alimentaire', famille: famille};
                    }
                }
            }
        }
        ...
        const mappings = {
            'Hygiène': ['hygiene', 'soap', 'shampoo', 'toothpaste', 'hygiène', 'gel', 'deodorant'],
            'Entretien': ['cleaning', 'detergent', 'household', 'entretien', 'bleach', 'javel'],
            'Bébé': ['baby', 'diaper', 'bébé', 'couche', 'infant']
        };
        ...
        for (let [famille, keywords] of Object.entries(mappings)) {
            for (let tag of tags) {
                for (let keyword of keywords) {
                    if (tag.includes(keyword)) {
                        return {rayon: 'Non Alimentaire', famille: famille};
                    }
                }
            }
        }
...
function show_api_preview_dialog(frm, api_data) {
    ...
    let preview_html = `
        <div style="padding: 10px;">
            <div style="margin-bottom: 20px;">
                <span class="indicator ${source_color}">
                    Source: <strong>${source_label}</strong>
                </span>
            </div>
            ...
            <table class="table table-bordered" style="margin-top: 15px;">
                ...
                <tr>
                    ...
                    <td>${api_data.barcode}</td>
                </tr>
                <tr>
                    <td><strong>Nom produit</strong></td>
                    <td>${api_data.product_name_api || '<em>Non trouvé</em>'}</td>
                </tr>
                <tr>
                    <td><strong>Marque</strong></td>
                    <td>${api_data.brands || '<em>Non trouvé</em>'}</td>
                </tr>
                <tr>
                    <td><strong>Quantité</strong></td>
                    <td>${api_data.quantity_text || '<em>Non trouvé</em>'}</td>
                </tr>
                <tr>
                    <td><strong>Poids normalisé</strong></td>
                    <td>${api_data.poids_kg ? api_data.poids_kg + ' Kg' : '<em>Non calculé</em>'}</td>
                </tr>
                <tr>
                    <td><strong>Rayon suggéré</strong></td>
                    <td>${api_data.suggested_rayon || '<em>Aucun</em>'}</td>
                </tr>
                <tr>
                    <td><strong>Famille suggérée</strong></td>
                    <td>${api_data.suggested_famille || '<em>Aucune</em>'}</td>
                </tr>
                <tr style="background-color: #f9f9f9;">
                    <td><strong>💰 Prix moyen</strong></td>
                    <td>${api_data.prix_moyen_eur ? api_data.prix_moyen_eur + ' EUR' : '<em>Non trouvé</em>'}</td>
                </tr>
            </table>
            
            ${api_data.image_url ? `
                <div style="margin-top: 15px; text-align: center;">
                    <img src="${api_data.image_url}" style="max-width: 200px; max-height: 200px; border: 1px solid #ddd; padding: 5px;">
                </div>
            ` : ''}
            ...
            ${!api_data.found ? `
                <div class="alert alert-warning" style="margin-top: 15px;">
                    ...
                    Vous devrez saisir les informations manuellement.
                </div>
            ` : ''}
        </div>
    `;
    ...
    let d = new frappe.ui.Dialog({
        ...
        size: 'large',
        primary_action_label: api_data.found ? __('✅ Utiliser ces données') : __('✏️ Saisir manuellement'),
        primary_action: function() {
            apply_api_data(frm, api_data);
            d.hide();
        },
...
/**
 * Appliquer données au formulaire
 */
function apply_api_data(frm, api_data) {
    
    if (api_data.product_name_api) {
        frm.set_value('nom_article', api_data.product_name_api);
    }
    
    if (api_data.brands) {
        frm.set_value('brands', api_data.brands);
    }
    
    if (api_data.quantity_text) {
        frm.set_value('quantity_text', api_data.quantity_text);
    }
    
    if (api_data.poids_kg) {
        frm.set_value('poids_kg', parseFloat(api_data.poids_kg));
    }
    
    if (api_data.image_url) {
        frm.set_value('image_url', api_data.image_url);
    }
    
    // Remplir Rayon + Famille
    if (api_data.suggested_rayon) {
        frm.set_value('rayon', api_data.suggested_rayon);
    }
    
    if (api_data.suggested_famille) {
        setTimeout(function() {
            frm.set_value('famille', api_data.suggested_famille);
        }, 200);
    }
    
    if (api_data.prix_moyen_eur) {
        frm.set_value('prix_moyen_eur', parseFloat(api_data.prix_moyen_eur));
    }
    
    if (api_data.source) {
        frm.set_value('api_source', api_data.source);
    }
    
    frappe.show_alert({
        message: __('✅ Données appliquées !'),
        indicator: 'green'
    }, 3);
}
